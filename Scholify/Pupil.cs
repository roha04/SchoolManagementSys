...
namespace Scholify
{
    public class Pupil
    {
        ...
        private static HashSet<int> generatedPupilIds = new HashSet<int>();
        private static string[] genders1 = { "чоловіча", "жіноча" };
        private static Random random = new Random();
        private static int GenerateRandomPupilId()
        {
            int maxPupilId = 10000;

            while (true)
            {
                int randomPupilId = random.Next(1, maxPupilId + 1);

                if (!generatedPupilIds.Contains(randomPupilId))
                {
                    generatedPupilIds.Add(randomPupilId);
                    return randomPupilId;
                }
            }
        }
        ...
        private static string generateGender()
        {
            while (true)
            {
                int index_genders = random.Next(genders1.Length);
                string genders = genders1[index_genders];
                return genders;
            }
        }
        public static DateTime GetBirthDate()
        {
            var minDate = new DateTime(2007, 01, 01);
        ...
        public List<int> GetAllUserIds(NpgsqlConnection connection)
        {
            ...
            using (NpgsqlCommand selectSubjectIdsCommand = new NpgsqlCommand($"SELECT user_id FROM users {"WHERE role = 'pupil'"}", connection))
            using (NpgsqlDataReader reader = selectSubjectIdsCommand.ExecuteReader())
            {
                while (reader.Read())
                {
        ...
        public void InsertPupils(int numberOfPupils)
        {
            using (var connection = new NpgsqlConnection("Server=localhost;Port=5433;Database=Scholify;User Id=postgres;Password=Rohasuper25;"))
            {
                ...
                {
                    int teacher_id = GenerateRandomPupilId();
                    ...
                    var command = new NpgsqlCommand("INSERT INTO pupils (pupil_id,first_name,middle_name,last_name," +
                        "gender,birthday,address,phone_number,class_id,user_id)" +
                        "VALUES (@pupil_id,@first_name,@middle_name,@last_name,@gender,@birthday,@address,@phone_number," +
                        "@class_id,@user_id)", connection);

                    command.Parameters.AddWithValue("@pupil_id", teacher_id);
